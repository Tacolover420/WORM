...
// Copyright (c) 2016-2018 The WORM developers
...
public:
    ...
    bool WriteZWORMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZWORMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZWORMSeed_deprecated(uint256& seed);
    bool EraseZWORMSeed();
    bool EraseZWORMSeed_deprecated();

    bool WriteZWORMCount(const uint32_t& nCount);
    bool ReadZWORMCount(uint32_t& nCount);
