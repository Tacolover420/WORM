...
CzWORMWallet::CzWORMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zworm seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZWORMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZWORMSeed_deprecated()) {
                    LogPrintf("%s: Updated zWORM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zworm seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zworm wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzWORMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZWORMCount(nCountLastUsed);
    else if (!walletdb.ReadZWORMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzWORMWallet::Lock()
{
    seedMaster = 0;
}

void CzWORMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzWORMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZWORM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzWORMWallet::LoadMintPoolFromDB()
{
...
void CzWORMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzWORMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzWORMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zwormTracker->HasPubcoinHash(pMint.first)) {
...
bool CzWORMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZWORM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zwormTracker which also adds to database
    pwalletMain->zwormTracker->Add(dMint, true);
...
void CzWORMWallet::SeedToZWORM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzWORMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzWORMWallet::GenerateDeterministicZWORM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzWORMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzWORMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzWORMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
