...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 WORM.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzWORMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzWORMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzWORMReset.connect(boost::bind(NotifyzWORMReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzWORMReset.disconnect(boost::bind(NotifyzWORMReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zwormTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
