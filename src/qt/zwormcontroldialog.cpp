...
#include "ui_zwormcontroldialog.h"
...
std::set<std::string> ZWormControlDialog::setSelectedMints;
std::set<CMintMeta> ZWormControlDialog::setMints;

bool CZWormControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZWormControlDialog::COLUMN_DENOMINATION || column == ZWormControlDialog::COLUMN_VERSION || column == ZWormControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZWormControlDialog::ZWormControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZWormControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZWormControlDialog::~ZWormControlDialog()
{
    delete ui;
}

void ZWormControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWormControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZWormControlWidgetItem* itemDenom(new CZWormControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZWormControlWidgetItem *itemMint = new CZWormControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zWORM seed used to mint this zWORM is not the same as currently hold in the wallet";
            else
...
void ZWormControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWormControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZWorm_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWormControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZWormControlDialog::GetSelectedMints()
{
...
void ZWormControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
