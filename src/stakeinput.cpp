...
CZWormStake::CZWormStake(const libzerocoin::CoinSpend& spend)
{
...
int CZWormStake::GetChecksumHeightFromMint()
{
...
int CZWormStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZWormStake::GetChecksum()
{
    return nChecksum;
}
...
// 100 blocks deep.
CBlockIndex* CZWormStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
CAmount CZWormStake::GetValue()
{
    return denom * COIN;
}

//Use the first accumulator checkpoint that occurs 60 minutes after the block being staked from
bool CZWormStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CZWormStake::GetUniqueness()
{
...
bool CZWormStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZWormStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CZWormStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZWormStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
...
//!WORM Stake
bool CWormStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CWormStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CWormStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CWormStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CWormStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CWormStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CWormStake::GetUniqueness()
{
...
CBlockIndex* CWormStake::GetIndexFrom()
{
    uint256 hashBlock = 0;
    CTransaction tx;
