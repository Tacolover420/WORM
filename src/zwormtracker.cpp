...
CzWORMTracker::CzWORMTracker(std::string strWalletFile)
{
...
CzWORMTracker::~CzWORMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzWORMTracker::Init()
{
...
bool CzWORMTracker::Archive(CMintMeta& meta)
{
...
bool CzWORMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzWORMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzWORMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzWORMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzWORMTracker::GetSerialHashes()
{
...
CAmount CzWORMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzWORMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzWORMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzWORMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzWORMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzWORMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzWORMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzWORMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzWORMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzWORMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzWORMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzWORMWallet* zWORMWallet)
{
    bool iszWORMWalletInitialized = (NULL != zWORMWallet);
    CMintMeta meta;
    ...
    if (! iszWORMWalletInitialized)
        zWORMWallet = new CzWORMWallet(strWalletFile);
    meta.isSeedCorrect = zWORMWallet->CheckSeed(dMint);
    if (! iszWORMWalletInitialized)
        delete zWORMWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzWORMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzWORMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzWORMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzWORMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzWORMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzWORMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzWORMWallet* zWORMWallet = new CzWORMWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zWORMWallet);
        }
        delete zWORMWallet;
        LogPrint("zero", "%s: added %d dzworm from DB\n", __func__, listDeterministicDB.size());
    }
