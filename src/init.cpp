...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZwormThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the WORM and zWORM money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in WORM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, worm, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all WORM specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-wormstake=<n>", strprintf(_("Enable or disable staking functionality for WORM inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zwormstake=<n>", strprintf(_("Enable or disable staking functionality for zWORM inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzworm=<n>", strprintf(_("Enable automatic wallet backups triggered after each zWORM minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zwormbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zWORM backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizewormamount=<n>     " + strprintf(_("Keep N WORM anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The WORM Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. WORM Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. WORM Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //WORM: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //WORM specific: zerocoin and spork DB's
                ...
                // WORM: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of WORM Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart WORM Core to complete") << "\n";
        ...
        zwalletMain = new CzWORMWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zWORMWallet
        uiInterface.InitMessage(_("Syncing zWORM wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZWormBackups = GetBoolArg("-backupzworm", true);
        pwalletMain->setZWormAutoBackups(fEnableZWormBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zwormTracker->Init();
...
//    if (nAnonymizeWormAmount < 2) nAnonymizeWormAmount = 2;
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZwormThread(threadGroup);
    }
